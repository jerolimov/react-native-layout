...
const reorderSubviews = (subviews: any[]) => {
  ...
  const horizontalStartIndex = subviews.findIndex(
    subview => subview.type === Left || subview.type === Right
  );
  const verticalStartIndex = subviews.findIndex(
    subview => subview.type === Top || subview.type === Bottom
  );

  if (horizontalStartIndex !== -1 || verticalStartIndex !== -1) {
    const isHorizontal =
      (horizontalStartIndex < verticalStartIndex &&
    ...
    for (var i = startIndex; i < subviews.length; i++) {
      if (
        (isHorizontal && subviews[i].type === Left) ||
        (!isHorizontal && subviews[i].type === Top)
      ) {
        before.push(subviews[i]);
      } else if (
        (isHorizontal && subviews[i].type === Right) ||
        (!isHorizontal && subviews[i].type === Bottom)
      ) {
    ...
    const content = subviews
      .slice(i)
      .filter(subview => subview.type !== Overlay);
    const overlays = subviews
      .slice(i)
      .filter(subview => subview.type === Overlay);

    const reordered = skipped
      .concat(before)
